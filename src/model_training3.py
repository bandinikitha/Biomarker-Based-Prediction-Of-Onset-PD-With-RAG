# from gayathri github
# ✅ Added these two lines (non-GUI backend fix)
...
LEAKY_PREFIXES = ("convert", "diagnos", "dx", "parkinson", "pd_",
                  "event", "status", "group", "cohort", "follow", "case", "control")
leaky = [c for c in X.columns if c.lower().startswith(LEAKY_PREFIXES)]
extra_ids = [c for c in ["PATNO", "subject_id",
                         "baseline_date_final"] if c in X.columns]
...
for name, pipeline in pipelines.items():
    scores = cross_val_score(pipeline, X_train_scaled,
...
best_name = max(
    ...
    key=lambda k: cross_val_score(
        best_models[k], X_train_scaled, y_train, cv=5, scoring="roc_auc").mean()
)
...
# ===============================
# Step 13.5: Dashboard metrics export
# ===============================
...
quick_metrics = {
    ...
    "brier": float(brier),
    "ece_10q": float(np.abs(prob_true - prob_pred).mean()),  # simple ECE
    "pos_rate": float(sum(y_test) / len(y_test)),
    "eval": "holdout"
}
with open("reports/quick_checks.json", "w") as f:
    json.dump(quick_metrics, f, indent=2)
print("✅ Saved reports/quick_checks.json")

# --- Subgroup metrics (optional: age/gender if available) ---
if any(col.lower().startswith(("age", "gender", "sex")) for col in X.columns):
    subgroup_cols = [c for c in X.columns if c.lower(
    ).startswith(("age", "gender", "sex"))]
    subgroup_df = []
    for col in subgroup_cols:
        for group in X_test[col].dropna().unique():
            mask = X_test[col] == group
            if mask.sum() > 5:
                auroc_sub = auc(*roc_curve(y_test[mask], y_proba[mask])[:2])
                subgroup_df.append(
                    {"subgroup": f"{col}={group}", "auroc": auroc_sub})
    pd.DataFrame(subgroup_df).to_csv(
        "reports/subgroup_metrics.csv", index=False)
    print("✅ Saved reports/subgroup_metrics.csv")
...
# ===============================
# Step 15: Generate reports folder for dashboard
# ===============================

X_test = X_test.reset_index(drop=True)
y_test = y_test.reset_index(drop=True)

subgroups = ["age_at_baseline", "RBD_slope", "UPSIT_slope"]
records = []

for col in subgroups:
    if col not in X_test.columns:
        print(f"⚠️ Column '{col}' not found in X_test — skipping.")
        continue

    grouped = X_test.groupby(col)
    for val, group in grouped:
        ...
        y_pred = y_proba[idx]  # This is safe now

        if len(np.unique(y_true)) < 2:
            # Avoid ROC computation errors when only one class present
            print(f"⚠️ Skipping subgroup {col}={val} (only one class present)")
            continue

        fpr, tpr, _ = roc_curve(y_true, y_pred)
        roc_auc = auc(fpr, tpr)
        prec, rec, _ = precision_recall_curve(y_true, y_pred)
        auprc = np.trapz(prec, rec)
...
os.makedirs("reports", exist_ok=True)
subgroup_df.to_csv("reports/subgroup_metrics.csv", index=False)
print("✅ Saved reports/subgroup_metrics.csv")
