...
from src.rag_module import generate_rag_reports
...
# used for medians & template only
DATASET_CSV = DATA_DIR / "final_dataset_enhanced.csv"
...
def load_artifacts():
    ...
    if not PIPE_PATH.exists() or not SCALER_PATH.exists():
        raise FileNotFoundError(
            "Model or scaler not found in ./models. Please run training first.")
    ...
    if FEATS_PATH.exists():
        ...
        if not DATASET_CSV.exists():
            raise FileNotFoundError(
                "feature_names.json missing and dataset not found to infer features.")
        df_tmp = pd.read_csv(DATASET_CSV, nrows=5)
        features = [c for c in df_tmp.columns if c not in (
            "convert_84m", "PATNO")]
...
def update_status(reminder_id: int, new_status: str):
    with sqlite3.connect(REMINDERS_DB) as conn:
        conn.execute("UPDATE reminders SET status = ? WHERE id = ?",
...
# --- Assume the following imports are present in the full script ---
# from src.rag_module import generate_rag_reports
# from __main__ import load_artifacts, training_statistics, preprocess_for_model, predict_dataframe, shap_explainer, SHAP_AVAILABLE, MODELS_DIR, REPORTS_DIR, PLOTS_DIR, DATA_DIR
# --- End assumed imports ---

# NOTE: For this code to run, ensure 'from src.rag_module import generate_rag_reports'
# is present at the top of your main Streamlit file.
def section_predict():
    ...
    with colL:
        ...
        st.caption(
            ...
            "Upload a CSV containing the model features. We'll align columns automatically.")
        ...
        if uploaded is not None:
            ...
            try:
                ...
                patno = df_in.get(
                    ...
                    "PATNO") if "PATNO" in df_in.columns else None

                preds = predict_dataframe(

                    df_in, features, medians, scaler, pipe)
                ...
                if SHAP_AVAILABLE and model is not None:
                    ...
                    st.subheader(
                        ...
                        "Per-patient explanation (SHAP) & RAG Reports")

                    # Build an aligned, preprocessed matrix again for SHAP

                    X, Xs = preprocess_for_model(

                        df_in, features, medians, scaler)

                    # Labels for selector

                    index_labels = (

                        patno.astype(str).values if patno is not None else X.index.astype(
                    ...
                    sel = st.selectbox(
                        ...
                        "Select a patient row for SHAP explanation and RAG report",

                        options=list(range(len(X))),

                        format_func=lambda i: (

                            f"Row {i} (PATNO={index_labels[i]})" if i < len(

                                index_labels) else f"Row {i}"
                    ...
                    try:
                        ...
                        if expl is None:
                            ...
                            st.info(
                                ...
                                "SHAP explainer not available for this model.")
                            ...
                            if sel < 0 or sel >= len(X):
                                ...
                                sv_raw = expl.shap_values(

                                    row, check_additivity=False)
                                ...
                                patient_id = index_labels[sel]

                                prediction_row = preds.iloc[sel]

                                # Features dictionary for RAG (includes all inputs)

                                all_patient_features = dict(

                                    X.iloc[sel].fillna('N/A'))

                                # Prediction dictionary for RAG

                                prediction_data_for_rag = {

                                    "label": "High Risk" if prediction_row["prediction"] == 1 else "Low Risk",

                                    "probability": prediction_row["probability"],

                                    # Pass SHAP values as importance

                                    "importance": dict(zip(top_names, top_vals)),

                                }

                                # =====================================

                                # 🌟 RAG REPORT GENERATION INTEGRATION 🌟
                                ...
                                try:
                                    ...
                                    # Call the RAG function with the patient's data and SHAP results

                                    report_type = "Detailed Doctor & Patient Reports"
                                    detail_level = "Detailed"

                                    # Call the RAG function with the patient's data and SHAP results
                                    reports = generate_rag_reports(
                                        features=all_patient_features,
                                        prediction=prediction_data_for_rag,
                                        clinician_notes=f"Patient ID: {patient_id}. Model is {prediction_data_for_rag['label']} with {prediction_data_for_rag['probability']:.4f} probability.",
                                        top_k=5,

                                    )

                                    # Tabs for displaying reports
                                    raw_metadata = reports["retrieved"]
                                    unique_sources = set()
                                    for meta in raw_metadata:
                                        title = meta.get(
                                            'title', 'Unknown Source')
                                        page = meta.get('page', 'N/A')
                                        # Clean up common non-ASCII characters that make the output messy
                                        clean_title = title.split('...')[0].replace(
                                            '\u2013', '-').replace('\u2018', "'").replace('\u2019', "'").replace('\u201a', '')
                                        source_key = f"📄 {clean_title} (Page {page})"
                                        unique_sources.add(source_key)
                                    source_list = "\n".join(
                                        [f"* {s}" for s in sorted(list(unique_sources))])
                                    tab_d, tab_p = st.tabs(
                                        ["👨‍⚕️ Doctor's Report", "🧑‍🦱 Patient Report"])

                                    with tab_d:
                                        st.markdown(reports["doctor_report"])
                                        st.caption("---")
                                        st.caption(
                                            "📚 **References Used (Deduplicated):**")
                                        # 👇 The corrected line: displays the clean list
                                        st.markdown(source_list)

                                    with tab_p:
                                        st.markdown(reports["patient_report"])
                                        st.caption("---")
                                        st.caption(
                                            "📚 **References Used (Deduplicated):**")
                                        # 👇 The corrected line: displays the clean list
                                    ...
                                    # Catch the Groq API Key error or other runtime issues

                                    st.error(

                                        f"RAG Report Generation Failed (Check GROQ_API_KEY): {e}")

                                except Exception as e:

                                    st.error(

                                        f"An unexpected error occurred during report generation: {e}")
                                ...
                                ax.set_xlabel(
                                    ...
                                    "SHAP value (impact on log-odds)")

                                ax.set_title(

                                    f"Top feature contributions for PATNO={patient_id}")
                    ...
                    st.info(
                        ...
                        "Install the 'shap' package and ensure the model is loaded to enable explanations/reports.")
...
def section_dashboard():
    ...
    try:
        ...
        if metrics is None:
            st.warning(
                "No reports/quick_checks.json found. Run training first.")
            return
        # Headline metrics
...
def section_reminders():
    ...
    for idx, (tab, filt) in enumerate(zip(tabs, status_filters)):
        with tab:
            ...
            if df.empty:
                ...
                with colA:
                    sel_to_complete = st.multiselect(
                        ...
                        key=f"complete_select_{idx}",  # 👈 unique key
                    )
                    if st.button("Mark Completed", key=f"complete_btn_{idx}") and sel_to_complete:
                        for rid in sel_to_complete:
                            update_status(int(rid), "completed")
                        st.success("✅ Selected reminders marked completed.")
                        st.rerun()
                ...
                with colB:
                    sel_to_sched = st.multiselect(
                        ...
                        options=df["id"].tolist(),
                        key=f"sched_select_{idx}",  # 👈 unique key
                    )
                    if st.button("Mark Scheduled", key=f"sched_btn_{idx}") and sel_to_sched:
                        for rid in sel_to_sched:
                            update_status(int(rid), "scheduled")
                        st.success("🔄 Selected reminders rescheduled.")
                        st.rerun()
